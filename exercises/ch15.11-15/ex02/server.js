...
// 確率的に問題を引き起こすミドルウェア
async function chaosMiddleware(_url, _req, res) {
  const lottery = Math.random();
  if (lottery < 1 / 3) {
    // 1/3 の確率で 500 エラー
    console.error("drew the internal server error");
...
async function main() {
  ...
  http
    .createServer(async function (req, res) {
      await routes(
        ["GET", "/api/tasks", listTasksHandler, authz, chaos],
        ["GET", "/api/tasks/{id}", getTaskHandler, authz, chaos],
